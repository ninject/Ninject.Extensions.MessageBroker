...
namespace Ninject.Extensions.MessageBroker.Model.Channels
{
    ...
    public class StandardMessageChannel : DisposableObject, IMessageChannel
    {
        ...
        public override void Dispose( bool disposing )
        {
            ...
            {
                foreach ( IMessagePublication messagePublication in _publications )
                {
                    messagePublication.Dispose();
                }
                foreach ( IMessageSubscription messageSubscription in _subscriptions )
                {
        ...
        public bool RemoveSubscription( object subscriber, MethodInjector injector )
        {
            Ensure.NotDisposed( this );
            ...
            lock ( _subscriptions )
            {
                foreach ( IMessageSubscription subscription in _subscriptions )
                {
                    if ( ( subscription.Subscriber == subscriber ) &&
                         ( subscription.Injector.Method == injector.Method ) )
                    {
